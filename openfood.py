...
def turbo_prep():
 ...
 #now only works for herrath
  ...
  addr_amount_dict = {}
  addr_amount_dict[wallet_delivery_date['address']] = WALLET_DELIVERY_DATE_THRESHOLD_UTXO_VALUE*1.5
  addr_amount_dict[wallet_pon['address']] = WALLET_PON_THRESHOLD_UTXO_VALUE*1.5
  addr_amount_dict[wallet_tin['address']] = WALLET_TIN_THRESHOLD_UTXO_VALUE*1.5
  addr_amount_dict[wallet_prod_date['address']] = WALLET_PROD_DATE_THRESHOLD_UTXO_VALUE*1.5
  addr_amount_dict[wallet_julian_start['address']] = WALLET_JULIAN_START_THRESHOLD_UTXO_VALUE*1.5
  addr_amount_dict[wallet_julian_stop['address']] = WALLET_JULIAN_STOP_THRESHOLD_UTXO_VALUE*1.5
  addr_amount_dict[wallet_origin_country['address']] = WALLET_ORIGIN_COUNTRY_THRESHOLD_UTXO_VALUE*1.5
...
def sendToBatch(wallet_name, threshold, batch_raddress, amount, integrity_id):
    ...
    if (send is None):
        ...
        #no_wait
        amountXtra = amount*3
        amount = treshold-amount + amountXtra
...
def sendToBatchJDS(batch_raddress, jds, integrity_id):
    ...
    funding_txid = fund_offline_wallet2(wallet['address'], float(jds))
    return send_batch # TXID
...
def sendToBatchJDE(batch_raddress, jde, integrity_id):
    ...
    #no_wait
    wallet = getOfflineWalletByName(WALLET_JULIAN_STOP)
    funding_txid = fund_offline_wallet2(wallet['address'], float(jde))
    return send_batch # TXID
